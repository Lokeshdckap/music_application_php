...
<section>
          <?php if(isset($_SESSION['Song Already Exists'])):?>
            <p class="bg-danger text-white"><?php echo $_SESSION['Song Already Exists']?></p>
            <?php endif;?>
                ...
                <div class="userContents" style="display: flex;">
                      <div class="card">
                      <div class="card-body">
                      <?php if(!empty($allPlaySongs)):?>
                       <h4> Your Playlists Are There for <?php echo $allPlaySongs[0]['playlist_name']?></h4>
                      <?php foreach($allPlaySongs as $datas):?>
                     <form action="#" method="POST">
                    <ul>
                      ...
                      <li><b>Date to added</b> : <?php echo date('d-m-Y',strtotime($datas['created_at']));?></li>
                      <img src="../<?php echo $datas['image_path']?>" width="100px" hieght="100px">
                      <audio controls src="../<?php echo $datas['file_name']?>"></audio>
                      <button type="submit" class="edit btn btn-danger" name="song_id" value="<?php echo $datas['id'];?>">Remove</button>
                    </ul>
                    </form>
                    <?php endforeach;?>
                    <?php else :?>
                    <h3>No Playlists are there</h3>
                    <?php endif; ?>
                      </div>
                    </div>
                    </div> 
        ...
        <div class="row">
                    <div class="userContents" style="display: flex;">
                      <div class="card">
                      <div class="card-body">
                        <?php $_SESSION['play'] = $_POST['playlist'] ?>
                    <h4>Recommended Songs add to your playlist</h4>
                     <form action="/addToPlay" method="POST">
                      ...
                      <img src="../<?php echo $datas['image_path']?>" width="100px" hieght="100px">
                      <audio controls src="../<?php echo $datas['file_name']?>"></audio>
                      <button type="submit" class="edit btn btn-primary" name="song_id" value="<?php echo $datas['id'];?>">Add to your playlist</button>
                      <input type="hidden" name="playlist_id" value="<?php echo $_SESSION['play'];?>">
                    </ul>
                    <?php endforeach;?>
                    </form>
                      </div>
